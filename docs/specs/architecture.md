...
This document is the architectural source of truth for the Personal OS. It is grounded in `Personal OS PRD.md` (the requirements) and `Personal OS Research Report.html` (the landscape and recommended approach). It records the consequential decisions, the runtime model, the components, the data model, the integration contracts, a vertical-slice build plan, the non-functional thresholds, and a register of the now-resolved technical decisions. As of v1.1 all ten open technical questions have been resolved; their answers are recorded in §8 and propagated throughout this document.
